...
-- Створення таблиці Винуватець
CREATE TABLE Culprit (
    ID_Culprit INT PRIMARY KEY IDENTITY(1,1),
    ID_Related INT NOT NULL, -- Ідентифікатор водія або пішохода
    ID_Accident INT NOT NULL FOREIGN KEY REFERENCES Accident(ID_Accident) ON DELETE CASCADE,
    Type VARCHAR(20) NOT NULL CHECK (Type IN ('Driver', 'Pedestrian')) -- Тип винуватця
);
